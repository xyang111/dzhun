...
⚠️ 该客户已有多款可申请产品，禁止建议"暂停所有申请""停止点击任何链接"等 D 级建议。重点是优化申请顺序和额度上限，而非等待。
① key_risk：用"现在最高X万，优化后可达Y万，差Z万额度"这个句式，Z必须是具体数字（从candidateSummary最高额度估算，优化后+30%-50%）。
② optimization：1-3步，每步必须含相对时间节点（"立即""1个月后""3个月后"，禁止输出绝对年月）。重点：把"等查询冷却"换算成"等N个月后申请，通过率从X%升到Y%"；把"结清小贷"换算成"结清后额度上限多X万"。time字段用相对时间，unlock字段必须含金额或额度数字。
