...
function calcBlastRisk(qRecords,baseDate) {
  ...
  if(risk>=80){badge='正常';cls='br-safe';tip='查询频率正常，可正常申请各类产品。';}
    ...
    tip = isHuahu
      ? `半年累计 ${q6m_br} 次查询已达花户红线，主流银行风控会扣分；建议进入查询冷却期再申请股份制/国有大行，急用钱请联系顾问筛选对查询不敏感的渠道。`
      : '近期查询偏多，建议暂停查询一段时间再申请，当前申请银行类产品通过率明显下降。';
  }
  else if(risk>=30){
    badge=isSerious?'严重花户':'爆查';cls='br-danger';
    tip = isSerious
      ? `半年累计 ${q6m_br} 次查询已达严重花户红线，主流银行风控直接拒贷；建议立即停止所有信贷申请，进入征信修复期再评估。`
      : '近期查询次数过多！建议停止所有查询进入冷却期，银行类产品当前基本无法通过。';
  }
  else{badge='严重';cls='br-critical';tip='查询严重超标，银行风控判定资金压力较大，建议立即停止所有申请进入修复期，具体路径解锁后查看。';}
...
function renderMatchResult(r) {
  ...
  if (_currentAgent) {
    ...
    // 信用卡分期补充渠道：代理商分支也显示，与主站保持一致
    const _ccTipsAg = document.getElementById('ccInstallTips');
    if (_ccTipsAg) _ccTipsAg.style.display = 'block';
...
(async function initWechatShare() {
  ...
  try {
    ...
    wx.ready(function () {
      const shareData = {
        title:  '你的征信，该做次体检了 — 贷准 AI',
        desc:   'AI 读懂你的征信：负债结构 / 查询风险 / 优化建议。',
        link:   pageUrl,
        imgUrl: 'https://dzhun.com.cn/share-logo.png'
      };
